# dictionary
# key: value türünde veri saklanır, sıralanabilir, güncellenebilir, tekrarlanmaz
# sehirler = ['kocaeli','istanbul']
# plakalar = [41,34]

# # key - value
# print(plakalar[0], sehirler[0])
# print(plakalar[1], sehirler[1])

# print(plakalar[sehirler.index('istanbul')])
# print(plakalar[sehirler.index('kocaeli')])

# dictionary

plakalar = {
    'kocaeli': 41,
    'istanbul': 34,
    'izmir': 36
}

plakalar['izmir'] = 35

print(plakalar['kocaeli'])
print(plakalar['istanbul'])
print(plakalar['izmir'])

# dictionary uygulama
# Aşağıdaki öğrenci bilgilerini dictionary listesinde saklayınız.
# 101 Yiğit Bilgi 2010 (40,80,90)
# 102 Ada Bilgi   2012 (80,80,80)
# 103 Çınar Turan 2017 (70,70,70)
ogrenciler = {101: {"Ad": "Yiğit", "Soyad": "Bulut",
                    "Doğum tarihi": 2010, "Notlar": (40, 80, 90)},
              102: {"Ad": "Ada", "Soyad": "Bilgi",
                    "Doğum tarihi": 2012, "Notlar": (80, 80, 80)},
              103: {"Ad": "Çınar", "Soyad": "Turan",
                    "Doğum tarihi": 2017, "Notlar": (70, 70, 70)}}
# Klavyeden girilen öğrenci numarasına göre aşağıdaki cümleyi yazdırınız.
# 101 numaralı Yiğit Bilgi ismindeki öğrencinin yaşı 14 ve not ortalaması 70.
ogrenciNo = int(input("Öğrenci numarasını giriniz  : "))
ogrenci = ogrenciler[ogrenciNo]
# print(type(ogrenci))
ortalama = (ogrenci["Notlar"][0]+ogrenci["Notlar"][1]+ogrenci["Notlar"][2])/3
print(f"{ogrenciNo} numaralı {ogrenci["Ad"], ogrenci["Soyad"]} ismindeki öğrencinin yaşı {2024- ogrenci["Doğum tarihi"]} ve not ortalaması {ortalama}")
